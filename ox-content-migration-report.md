...
### 追記: モバイルサイドメニューのスクロール修正 (2026-08-27)

alpha.9 (#915) でモバイルのサイドメニューは「header〜footer 間のフルシートを
translateY で開閉し、内部を overflow-y: auto でスクロール」する設計に変わった。
custom.css に残っていた旧 holo 対策の `top: auto` がこの高さ制約を外し、
シートがコンテンツ全高(top が負値)になって内部スクロールが死んでいた。
holo の無スコープな `top: 0` は健在のためオーバーライド自体は必要で、値を
ベーステーマと同じ `top: var(--octc-header-height)` に変更して解消。
e2e にモバイルシートの内部スクロール検証を追加(33項目)。
